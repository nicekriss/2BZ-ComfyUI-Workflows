...
## ⚠ 라이선스 (상업 이용 주의)
Qwen-Image 2.1 모델은 이전 버전(Apache 2.0)과 달리 **[Qwen Research License](https://huggingface.co/Qwen/Qwen-Image-2.1/blob/main/LICENSE)** 로 공개됐습니다. 연구·평가 목적만 허용되고, 상업적으로 쓰려면 별도 상업 라이선스가 필요합니다.

- 생성한 이미지의 소유권은 사용자에게 있는 것으로 보이지만, FLUX.1 [dev]처럼 "출력물은 상업적으로 써도 된다"는 조항이 없습니다. 상업 목적으로 이미지를 만드는 것 자체가 라이선스에 걸릴 수 있습니다.
- **지금은 개인 테스트·학습용으로만 쓰고, 협찬·외주·판매용 작업에는 쓰지 마세요.** 상업 작업에는 Apache 2.0인 Qwen-Image 2512 / Qwen-Image-Edit 2511을 권합니다.
- 공식적으로 출력물 상업 이용이 명확해지면 이 안내를 갱신합니다. (2026-09-24 기준)
- 이 저장소의 워크플로 json과 문서는 MIT입니다. 모델 가중치는 포함하지 않습니다.
